...
DEF: A rapdoor func X -> Y is a triple of efficient algs (G, F, F^-1)
- G() gen for (pk,sk)
- F(pk, .) det alg that defines X -> Y
...
Essentially, the TDF is used to encrypt a random x, and then we use a normal encryption system.
...
Chose integers e,d s.t. e*d = 1 (mod phi(N))
...
Plugigng RSA into the ISO std, we have a real pub key encryption system!
